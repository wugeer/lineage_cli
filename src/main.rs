...
fn run_once(sql: &str, tables_mode: bool, json_mode: bool, pretty: bool) -> anyhow::Result<()> {
    if tables_mode {
        if json_mode {
            let infos = lineage_cli::analyze_sql_tables_detailed(sql)?;
            if pretty {
            ...
            let lines = lineage_cli::analyze_sql_tables(sql)?;
            for l in lines {
                println!("{}", l);
            }
        }
    } else if json_mode {
        let infos = lineage_cli::analyze_sql_lineage_detailed(sql)?;
        if pretty {
            println!("{}", serde_json::to_string_pretty(&infos)?);
        } else {
            println!("{}", serde_json::to_string(&infos)?);
        ...
        let lines = lineage_cli::analyze_sql_lineage(sql)?;
        for l in lines {
            println!("{}", l);
        }
    }
